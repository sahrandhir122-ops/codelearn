...
// ─────────────────────────────────────────────────────────────────────────────
// YouTubePlayer — full custom controls over an iframe embed
// ─────────────────────────────────────────────────────────────────────────────
function YouTubePlayer({ src, autoPlay }) {
  ...
  // Start playing=false — actual state comes from infoDelivery postMessage events.
  // This prevents the 3-second auto-hide timer firing before the user touches anything.
  ...
  const cmd = useCallback((func, args = []) => {
    iframeRef.current?.contentWindow?.postMessage(
      JSON.stringify({ event: "command", func, args }), "*"
    );
  }, []);

  // Receive state/time updates from YouTube (infoDelivery events)
  useEffect(() => {
    const onMsg = (e) => {
      if (!e.data) return;
      try {
        ...
        if (d.event === "infoDelivery" && d.info) {
          if (!seekingRef.current && typeof d.info.currentTime === "number")
            setCurrent(d.info.currentTime);
          ...
          if (typeof d.info.volume === "number")
            setVolume(d.info.volume / 100);
          if (typeof d.info.muted === "boolean")
            setMuted(d.info.muted);
          if (typeof d.info.playbackQuality === "string" && d.info.playbackQuality !== "unknown")
            setQuality(d.info.playbackQuality);
        }
      } catch (_) {}
    };
  ...
  // Subscribe to YouTube info after iframe loads
  ...
  // Auto-hide: show on mouse move / pause, hide 3s after mouse leaves while playing
  ...
  // Always show controls while paused or popup open
  useEffect(() => {
    if (!playing || popup) { clearTimeout(hideTimer.current); setCtrlVis(true); }
  }, [playing, popup]);
  ...
  const toggleMute = () => {
    if (muted) { cmd("unMute"); setMuted(false); }
    else       { cmd("mute");   setMuted(true);  }
  };
  const handleVol = (e) => {
    const v = parseFloat(e.target.value);
    setVolume(v);
    cmd("setVolume", [Math.round(v * 100)]);
    if (v === 0) setMuted(true);
    else if (muted) { cmd("unMute"); setMuted(false); }
  };

  const applySpeed   = (s) => { setSpeed(s);   cmd("setPlaybackRate",  [s]); setPopup(null); };
  const applyQuality = (q) => {
    setQuality(q);
    cmd("setPlaybackQuality", [q === "auto" ? "default" : q]);
    setPopup(null);
  };

  // Friendly label for the quality button
  const qualityLabel = (q) => {
    const found = YT_QUALITIES.find(x => x.value === q);
  ...
  // Fullscreen: flex-col so iframe flex-1 fills remaining height, controls stay visible at bottom
  // Normal: just a black block, iframe has aspect-ratio, controls in flow below
  const wrapCls = isFs
    ? "bg-black w-full select-none flex flex-col"
    : "bg-black w-full select-none";
  ...
  return (
    ...
    >
      ...
      {/* ── Controls bar — in-flow below iframe, never hidden by iframe stacking context ── */}
      <div
        className={`transition-opacity duration-300 flex-shrink-0 ${ctrlVis ? "opacity-100" : "opacity-0"}`}
        style={{ background: "linear-gradient(to top, rgba(0,0,0,0.95), rgba(0,0,0,0.80))" }}
        onClick={(e) => e.stopPropagation()}
      >
        {/* Seek bar */}
        <div
          ref={seekBarRef}
          className="mx-4 mt-3 mb-3 h-1.5 rounded-full cursor-pointer relative group"
          style={{ background: "rgba(255,255,255,0.15)" }}
          onClick={(e) => { if (duration) seekTo(getSeekRatio(e)); }}
          onMouseDown={(e) => { seekingRef.current = true; if (duration) setCurrent(getSeekRatio(e) * duration); }}
          onMouseMove={(e) => { if (seekingRef.current && duration) setCurrent(getSeekRatio(e) * duration); }}
          onMouseUp={(e) => { if (seekingRef.current) { seekingRef.current = false; if (duration) seekTo(getSeekRatio(e)); } }}
        ...
        >
          <div className="absolute inset-y-0 left-0 rounded-full bg-primary" style={{ width: `${progress}%` }}>
            <div className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-1/2 w-3.5 h-3.5 rounded-full bg-white opacity-0 group-hover:opacity-100 transition-opacity"
        ...
        <div className="flex items-center gap-2 px-4 pb-3">
          ...
          <button onClick={toggleFs} title={isFs ? "Exit Fullscreen" : "Fullscreen"}
            className={`w-8 h-8 flex items-center justify-center rounded-lg transition-colors ${isFs ? "text-primary bg-primary/15" : "text-white/70 hover:text-white hover:bg-white/10"}`}>
            {isFs
